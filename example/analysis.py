...
plot_dir = os.getcwd() # current directory
if not os.path.exists(plot_dir):
    print "plot_dir path, '{}' doesn't exist!".format(plot_dir)
    raise AssertionError
################################################################################
# Tuple of 1D histogram init arguments
# format: (str NAME, str TITLE, int NBINS, float XMIN, float XMAX)
histargs = (
            ('ptl1','Leading lepton pT', 30, 0., 150.),
            ('ptl2','2nd lepton pT', 20, 0., 100.),
            ('mll' ,'Di-lepton invariant mass', 20, 85., 95.),
            ('ptj' ,'jet pT', 20, 0., 100.),
            )
################################################################################
class MyPySelector( TPySelector ):
    ...
    def Process( self, entry ):
        ...
        # Filling a histogram:
        # at least 2 leptons and 1 jet in event
        if evt.nlep >=2 and evt.njet >=1: 
            leps = pt_sort(evt.leptons) # sort jets according to pT
            lep1, lep2 = leps[0], leps[1] # leading leptons
            jets = pt_sort(evt.ljets) # sort jets according to pT
            jet1 = jets[0] # leading jet
            # fill histograms referencing by histo NAME declared above
            # lepton pt's
            self.histos['ptl1'].Fill(lep1.pt)
            self.histos['ptl2'].Fill(lep2.pt)
            # dilepton invariant mass
            mll = Minv(lep1, lep2) 
            self.histos['mll'].Fill(mll)
            # jet pt
            self.histos['ptj'].Fill(jet1.pt)
    ...
    def Terminate( self ):
        ...
        aname = os.path.basename(__file__).replace('.py','')
        tail = '{}_{}'.format(aname, name) if len(name)>0 else aname
        file_name = os.path.basename(fname).replace('.root','_{}.root'.format(tail))
        ...
        for h in histargs:
            ...
            hist.Write()
            if write_histos: 
                histname = '{}/{}_{}.dat'.format(plot_dir, file_name.replace('.root',''), hist.GetName())
                # write histogram data, normed=True to normalise to 1
                write_hist(hist, histname, normed=True)
        cur_dir.cd()
        out_file.Close()
        del out_file
        ...
        if write_histos: 
            print 'wrote histogram data to {}'.format(plot_dir)
        ########################################################################
