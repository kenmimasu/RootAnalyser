...
from RootAnalyser.smearing import smear_bjet
########################################################################
# Template Selector
########################################################################
avail_objects='''
...
histargs = (
            ('pt','Histogram I',50,0.,500.),
            ('smeared_pt','Histogram II',50,0.,500.),
            # ('hist3','Histogram III',50,0.,20.),
            )
################################################################################
class MyPySelector( TPySelector ):
    ...
    def Process( self, entry ):
        ...
        # Filling a histogram:
        # print evt
        if evt.nw >0: # at least 2 jets in event
            jet1 = evt.ws[0]
            # fill histograms referencing by histo NAME declared above
            self.histos['pt'].Fill(jet1.pt) 
            # smear jet
            jet1_sm = smear_bjet(jet1, seed=hash(jet1.pt))
            self.histos['smeared_pt'].Fill(jet1_sm.pt) 
            print jet1.pt, jet1_sm.pt
        # End analysis
        ########################################################################
        return 1
    ...
    def Terminate( self ):
        ...
        file_name= os.path.basename(fname).replace('.root','_analysis_cuts_{}.root'.format(name))
